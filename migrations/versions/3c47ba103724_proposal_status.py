...
from funnel.models import PROPOSAL_STATE
...
def upgrade():
    ...
    connection.execute(proposal.update().where(proposal.c.confirmed == True).values({proposal.c.status: PROPOSAL_STATE.CONFIRMED}))
    connection.execute(proposal.update().where(proposal.c.confirmed == False).values({proposal.c.status: PROPOSAL_STATE.SUBMITTED}))
...
def downgrade():
    ...
    ### end Alembic commands ###
    connection.execute(proposal.update().where(proposal.c.status == PROPOSAL_STATE.CONFIRMED).values({proposal.c.confirmed: True}))
