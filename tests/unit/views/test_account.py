...
def test_username_available(client, new_user):
    ...
    rv = client.post(endpoint, data={'username': 'should-be-available'})
    assert rv.status_code == 200
    assert rv.get_json() == {'status': 'ok'}

    # Taken usernames won't be available
    rv = client.post(endpoint, data={'username': new_user.username})
    ...
    rv = client.post(endpoint, data={'username': 'this is invalid'})
    assert rv.status_code == 200  # Validation failures are not 400/422
    assert rv.get_json() == {
        'status': 'error',
        'error': 'validation_failure',
