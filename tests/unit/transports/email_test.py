...
import pytest
from flask_mailman.message import EmailMessage, sanitize_address
...
def test_send_email_blocked() -> None:
    ...
    with patch.object(EmailMessage, 'send', return_value=None):
...
def test_send_email_hard_bouncing() -> None:
    ...
    with patch.object(EmailMessage, 'send', return_value=None):
        ...
        ea.mark_hard_fail()
        with pytest.raises(
            TransportRecipientError, match='This email address is bouncing'
        ):
