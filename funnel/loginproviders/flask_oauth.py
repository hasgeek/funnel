...
def get_etree():
    ...
    if _etree is None:
        try:
            ...
            try:
                from xml.etree import cElementTree as _etree  # noqa: N813
            except ImportError:
                try:
                    from xml.etree import ElementTree as _etree  # noqa: N813
                except ImportError:
...
def encode_request_data(data, format):  # noqa: A002
    if format is None:
        return data, None
    elif format == 'json':
...
class OAuthClient(oauth2.Client):
    def request_new_token(self, uri, callback=None, params={}):
        ...
        return httplib2.Http.request(
            self, uri, method='POST', body=body, headers=headers
        )
...
class OAuthException(RuntimeError):
    ...
    def __init__(self, message, type=None, data=None):  # noqa: A002
...
class OAuth(object):
    ...
    def remote_app(self, name, register=True, **kwargs):
        ...
        if register:
            assert name not in self.remote_apps, 'application already registered'
...
class OAuthRemoteApp(object):
    ...
    def __init__(
        ...
        self._consumer = oauth2.Consumer(self.consumer_key, self.consumer_secret)
    ...
    def request(
        ...
        return OAuthResponse(
            *client.request(url, method=method, body=data or '', headers=headers)
        )
    ...
    def generate_request_token(self, callback=None):
        ...
        resp, content = self._client.request_new_token(
            self.expand_url(self.request_token_url), callback, self.request_token_params
        )
        if not self.status_okay(resp):
            raise OAuthException(
                'Failed to generate request token', type='token_generation_failed'
            )
        data = parse_response(resp, content)
        if data is None:
            raise OAuthException(
                'Invalid token response from ' + self.name,
    ...
    def handle_oauth1_response(self):
        ...
        resp, content = client.request(
            '%s?oauth_verifier=%s'
            % (self.expand_url(self.access_token_url), request.args['oauth_verifier']),
            self.access_token_method,
        )
        data = parse_response(resp, content)
        if not self.status_okay(resp):
            raise OAuthException(
                'Invalid response from ' + self.name, type='invalid_response', data=data
            )
        return data
    ...
    def handle_oauth2_response(self):
        ...
        if self.access_token_method == 'POST':
            ...
            raise OAuthException(
                'Unsupported access_token_method: ' + self.access_token_method
            )
        data = parse_response(resp, content)
        if not self.status_okay(resp):
            raise OAuthException(
