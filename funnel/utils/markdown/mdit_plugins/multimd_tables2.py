...
def multimd_table_plugin(md: MarkdownIt, **options):
    ...
    def table_caption(state: StateBlock, silent, line):
        ...
        meta['label'] = meta['label'].lower().replace(r'\W+', '')
    ...
    def table(state: StateBlock, start_line, end_line, silent):
        ...
        if options['headerless']:
            ...
            table_dfa.update_transition(
                0x11100, {0x10000: 0x01100, 0x01000: 0x10010, 0x00100: 0x01100}
            )
            tr_token = Token('tr_placeholder', 'tr', 0)
            tr_token.meta = {}  # avoid trToken.meta.grp throws exception
        if not options['multibody']:
            table_dfa.update_transition(
                0x10010, {0x10000: 0x00000, 0x00010: 0x10010}
            ...
            if _type == 0x10000:
                ...
                if options['multiline']:
                    if tr_token.meta['multiline'] and table_dfa.mtr < 0:
                        ...
                        token.meta['mbounds'] = [
                            tk.meta['bounds']
                            for tk in table_token.meta['tr'][table_dfa.mtr :]
                        ]
                        token.map[1] = tr_token.map[1]
                        table_token.meta['tr'] = table_token.meta['tr'][
                            0 : table_dfa.mtr + 1
                        ]  # POTENTIAL POINT OF FAILURE; CHECK;
                        table_dfa.mtr = -1
            elif _type == 0x00001:
                tr_token.meta['grp'] |= 0x01
                table_dfa.grp = 0x10
        ...
        for tr_token_item in table_token.meta['tr']:
            ...
            for c in range(len(tr_token_item.meta['bounds'] - 1)):
                text = copy(
                    state.src[
                        tr_token_item.meta['bounds'][c]
                        + 1 : tr_token_item.meta['bounds'][c + 1]
                    ]
                )
                if text == '':
                    ...
                    left_token.attrSet(
                        'colspan',
                        2 if table_dfa.colspan is None else table_dfa.colspan + 1,
                    )
                    continue
                if (
                    options['rowspan']
                    and table_dfa.up_tokens[c]
                    and text.strip() == '^^'
                ):
                    left_token = Token('td_th_placeholder', '', 0)
                    continue
                ...
                if table_token.meta['sep']['aligns'][c]:
                    token.attrs['style'] = (
                        'text-align:' + table_token.meta['sep']['aligns'][c]
                    )
                ...
                if (
                    options['multiline']
                    and tr_token_item.meta['multiline']
                    and tr_token_item.meta['mbounds']
                ):
                    ...
                    for b in range(len(tr_token_item.meta['mbounds'])):
                        ...
                        text.extend(
                            copy(
                                state.src[
                                    tr_token_item.meta['bounds'][b][c]
                                    + 1 : tr_token_item.meta['bounds'][b][c + 1]
                                ]
                            )
                        )
                    ...
                    state.md.block.tokenize(
                        block_state, tr_token_item.map[0], block_state.lineMax
                    )
