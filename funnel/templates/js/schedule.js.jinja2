      ...
      <div>
        <div class="tabs-wrapper mui--hidden-xs mui--hidden-sm">
          <div class="tabs">
            ...
            <button class="tabs__item" :class="(view === 'calendar') ? 'tabs__item--active' : ''" @click="toggleView($event, 'calendar')">{{ gettext('Calender view') }}</button>
          </div>
        </div>
        <div class="schedule" v-for="schedule in schedules">
          <h2 class="mui--text-heading schedule__date" :class="[schedule.upcoming ? 'schedule__date--upcoming' : '']">{{ schedule.dateStr }}</h2>
