{% macro comments_tree() %}
  {% raw %}
    <div id="comments-wrapper" v-cloak>
      <a class="mui-btn mui-btn--small mui-btn--raised mui-btn--primary" :href="loginUrl" data-cy="login-btn" v-if="!isuserloggedin">{{ gettext('Login to leave a comment') }}</a>
      <p class="mui-panel mui--bg-accent" v-if="isuserloggedin && !isuserparticipant">{{ gettext('You need to be a participant to comment.') }}</p>
      <div class="mui--hidden-md mui--hidden-lg mui--hidden-xl" v-if="showmodal">
        <div class="mobile-nav mui--z1">
          <span aria-label="{{ gettext('Close') }}" class="mui--text-dark mobile-nav__icon" data-action="Close" @click="closeNewCommentForm($event)"><faicon :icon="'times'" :icon_size="'subhead'"></faicon></span><span class="mui--text-dark mobile-nav__headline">{{ formTitle }}</span>
        </div>
      </div>
      <a href="javascript:void(0)" class="post-form-block nounderline js-new-comment-form" :class="[commentForm ? 'mui--hide' : '']" @click="showNewCommentForm($event, 'comment_message')" aria-label="{{ gettext('Post comment') }}" data-cy="post-comment" v-if="isuserparticipant" role="button">
        ...
        <span class="comment-new-mobile mui--text-hyperlink mui--hidden-md mui--hidden-lg mui--hidden-xl"><faicon :icon="'comment-alt'" :icon_size="'subhead'" :css_class="'fa-icon--margin'"></faicon>{{ gettext('New comment') }}</span>
      </a>
      ...
      <ul class="mui-list--unstyled comment-wrapper" v-for="comment in comments">
        <comment :comment="comment" :user='user' :isuserparticipant="isuserparticipant" :iscommentmoderator="iscommentmoderator"></comment>
      </ul>
    </div>
    <script type="text/x-template" id="comment-template">
      <li class="comment" :class="[formOpened ? 'comment--modal' : '']"  :id="'c-' + comment.uuid_b58">
        <div class="mui--hidden-md mui--hidden-lg mui--hidden-xl" v-if="formOpened">
          <div class="mobile-nav mui--z1">
            <span aria-label="{{ gettext('Close') }}" class="mui--text-dark mobile-nav__icon" data-action="Close" @click="closeAllForms($event)"><faicon :icon="'times'" :icon_size="'subhead'"></faicon></span><span class="mui--text-dark mobile-nav__headline">{{ commentFormTitle }}</span>
          </div>
        </div>
        <div class="comment__header">
          <button class="mui-btn mui-btn--nostyle" aria-label="{{ gettext('Hide comment') }}" aria-expanded="true" v-if="!hide" @click="collapse(true)">
            <faicon :icon="'caret-circle-up'" :icon_size="'subhead'"></faicon>
          </button>
          <button class="mui-btn mui-btn--nostyle" aria-expanded="false" aria-label="{{ gettext('Collapse comment') }}" v-else @click="collapse(false)">
        ...
        <transition name="slide">
          <div class="comment__body" v-if="!hide">
            <div class="comment__body__inner">
              ...
              <div class="js-comment-form">
                ...
                <div class="mui-dropdown">
                  <a href="javascript:void(0)" class="link-icon project-links mui--text-light link comment__body__links" data-mui-toggle="dropdown" data-action="Share dropdown" data-cy="share-project"><faicon :icon="'share-alt-square'" :baseline=false :css_class="'fa5-icon--aligntop'"></faicon><span class="mui--hidden-xs mui--hidden-sm">{{ gettext('Share') }}</span></a>
                ...
                <div class="mui-dropdown" v-if="comment.urls.edit || comment.urls.delete">
                  <a href="javascript:void(0)" class="link-icon mui--text-light link comment__body__links" data-mui-toggle="dropdown"  data-cy="comment-menu"><faicon :icon="'ellipsis-v'" :baseline=false :css_class="'fa5-icon--aligntop'"></faicon><span class="mui--hidden-xs mui--hidden-sm">{{ gettext('More') }}</span></a>
                  <ul class="mui-dropdown__menu mui-dropdown__menu--hg-link mui-dropdown__menu--small" data-cy="share-dropdown">
                    <li><a href="javascript:void(0)" class="mui--text-body2 comment__body__links" data-cy="edit" v-if="comment.urls.edit" @click="activateSubForm($event, COMMENTACTIONS.EDIT, 'edit-comment-textarea-' + comment.uuid_b58)"><faicon :icon="'edit'" :icon_size="'caption'" :baseline=false :css_class="'mui--text-light fa-icon--margin'" ></faicon>{{ gettext('Edit') }}</a></li>
                    <li><a href="javascript:void(0)" class="mui--text-body2 comment__body__links" data-cy="delete" v-if="comment.urls.delete"  @click="activateSubForm($event, COMMENTACTIONS.DELETE)"><faicon :icon="'trash-alt'" :icon_size="'caption'" :baseline=false :css_class="'mui--text-light fa-icon--margin'"></faicon>{{ gettext('Delete') }}</a></li>
                  </ul>
                </div>
                ...
                <div class="ajax-form ajax-form--block" :class="[formOpened ? 'ajax-form--mob' : '']" v-if="deleteForm">
                  <form method="POST" accept-charset="UTF-8" class="mui-form" @submit.prevent="submitCommentForm('delete-comment-form-' + comment.uuid_b58, comment.urls.delete, 2)" :id="'delete-comment-form-' + comment.uuid_b58" data-cy="delete-form">
                    ...
                    <button type="submit" class="mui-btn mui-btn--small mui-btn--raised mui-btn--primary">Confirm</button>
            ...
            <ul class="mui-list--unstyled comment--children" v-for="reply_comment in comment.current_access_replies">
              <comment :comment="reply_comment" :user='user' :isuserparticipant="isuserparticipant" :iscommentmoderator="iscommentmoderator"></comment>
            </ul>
          </div>
        </transition>
      </li>
