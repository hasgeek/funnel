          ...
          <div class="grid">
            <div class="grid__col-12">
              <div class="tabs" id="scrollable-tabs">
                {{#each tabs}}
                  <p class="tabs__item mui--text-body2 mui--text-light mui--text-uppercase {{#if activeTab == type}}tabs__item--active{{/if}}" on-click="updateTabContent(type)">{{ label }} <span class="count-chip mui--text-caption">{{ count }}</span></p>
                {{/each}}
              </div>
            </div>
          </div>
            ...
            {{#if result == activeTab}}
              {{#results[result].items}}
                <div class="tab-content__results user grid__col-sm-12 {{#if result == 'project' || result == 'profile'}} grid__col-sm-6 grid__col-lg-4{{/if}}">
                  {{#if result == 'project'}}
                    <a href="{{ url }}" aria-label="{{ title }}" class="card card--upcoming clickable-card" data-action="View {{ title }}(search project)">
                      <div class="card__image-wrapper">
                        {{#if obj.bg_image }}
                          <img class="card__image" src="{{ obj.bg_image }}" alt="{{ title }}"/>
                        {{else}}
                            ...
                            <div class="card__calendar card__calendar--compact card__calendar--compact" aria-hidden="true">
                              <div class="calendar">
                                {{#obj.calendar_weeks_compact.weeks}}{{# { week: . } }}
                                  <p class="calendar__month-name {{#if week.upcoming && @index < 2}}calendar__month-name--upcoming calendar__month-name--upcoming--first calendar__month-name--latest calendar__month-name--unique{{/if}}">{{ week.month }} {{ week.year }}&nbsp;<span class="calendar__counting"></span></p>
                  ...
                  {{elseif result == 'profile'}}
                    <a href="{{ url }}" aria-label="{{ title }}" class="card card--upcoming clickable-card" data-action="View {{ title }}(search profile)">
                      <div class="card__image-wrapper">
                    ...
                    <div>
                      <div class="user__box">
                        ...
                        <div class="user__box__header comment__header__details__user">
                          <h2 class="mui--text-subhead mui--text-bold zero-top-margin zero-bottom-margin"><a href="{{ url }}">{{{ title_html }}}</a></h2>
                          <h3 class="mui--text-body2 zero-top-margin zero-bottom-margin">
                            {{#if obj.speaker.fullname }}<span>{{ obj.speaker.fullname }}</span> <span class="mui--text-light">in</span>{{/if}} <span>{{ obj.project.title }} </span>
                    ...
                    <div>
                      <div class="user__box">
                        ...
                        <div class="user__box__header comment__header__details__user">
                          ...
                          <p class="mui--text-body2 mui--text-light zero-bottom-margin">{{ formatDate(obj.created_at) }} {{#if obj.edited_at}}({{ gettext('edited %(date)s', {date: formatDate(obj.edited_at)}) }}) {{/if}}</p>
                        </div>
                      </div>
                      <p class="mui--text-body2 mui--text-light zero-top-margin snippets"><faicon icon='search' css_class='search-icon' baseline=false></faicon> {{{ snippet_html }}}</p>
                    </div>
              ...
              {{else}}
                <p class="mui--text-title">{{ gettext('No results found') }}</p>
              {{/}}
