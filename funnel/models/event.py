...
class Participant(BaseMixin, db.Model):
    ...
    def checkin_list(cls, event):
        ...
        """
        participant_list = db.session.query('id', 'order_no', 'fullname', 'email', 'company', 'twitter', 'puk', 'key', 'checked_in', 'badge_printed', 'ticket_type_titles').from_statement(text('''
            SELECT distinct(participant.id), sync_ticket.order_no, participant.fullname, participant.email, participant.company, participant.twitter, participant.puk, participant.key, attendee.checked_in, participant.badge_printed,
