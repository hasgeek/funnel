...
class Venue(UuidMixin, BaseScopedNameMixin, CoordinatesMixin, db.Model):
    ...
    project = db.relationship(Project,
        backref=db.backref('venues', lazy='dynamic', cascade='all, delete-orphan',
            order_by='Venue.seq', collection_class=ordering_list('seq')))
...
class VenueRoom(BaseScopedNameMixin, db.Model):
    ...
    venue = db.relationship(Venue,
        backref=db.backref('rooms', cascade='all, delete-orphan',
            order_by='VenueRoom.seq', collection_class=ordering_list('seq')))
